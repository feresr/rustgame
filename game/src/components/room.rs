        ...
        self.tiles.iter().enumerate().flat_map(|(y, row)| {
            row.iter().enumerate().filter_map(move |(x, tile)| {
                if matches!(tile, Tile::Solid { .. }) {
                    Some((x * TILE_SIZE, y * TILE_SIZE, tile))
                } else {
                    None
                }
            })
...
impl Room {
    ...
    pub fn empty(index : u32) -> Room {

        let x = (index % 4) as f32 * GAME_PIXEL_WIDTH as f32;
        let y = (index / 4) as f32 * GAME_PIXEL_HEIGHT as f32;
        let rect = RectF {
            x,
            y,
        ...
        Room {
            world_position: glm::Vec2::new(x, y),
            layers: vec![Layer {
                tileset_id: 0,
    ...
    pub fn from_level(level: &Level) -> Self {
        ...
        for layer in level.layer_instances.as_ref().unwrap() {
            match layer.layer_instance_type.as_str() {
                "Tiles" => {
                    let tiles =  [[Tile::Empty {}; GAME_TILE_WIDTH];  GAME_TILE_HEIGHT];
    ...
    pub fn prerender_normals(&mut self, batch: &mut Batch) {
        for layer in self.layers.iter().rev() {
            ...
            for (x, row) in layer.tiles.iter().enumerate() {
    ...
    pub fn prerender_colors(&mut self, batch: &mut Batch) {
        ...
        for layer in self.layers.iter().rev() {
            ...
            let tileset = Content::get().tilesets.get(&layer.tileset_id).expect("No tileset found");
